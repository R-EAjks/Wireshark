        ...
        case AFNUM_INET6:
            ...
            if (label)
            {
                ti = proto_tree_add_ipv6_format(tree, hf_ip6, tvb, offset, 2 + len,
                        &ipv6, "%s: %s", label, tvb_ip6_to_str(PNODE_POOL(tree), tvb, offset + 2));
            }
            else
            {
        ...
        case AFNUM_INET6:
            ...
            if (label)
            {
                ti = proto_tree_add_ipv6_format(tree, hf_ip6, tvb, offset, 4 + len,
                        &ipv6, "%s: %s", label, tvb_ip6_to_str(PNODE_POOL(tree), tvb, offset + 4));
            }
            else
            {
        ...
        case AFNUM_INET6:
            ...
            else
            {
                ti = proto_tree_add_ipv6_format_value(tree, hf_ip6, tvb, offset, 4 + len, &ipv6,
                                                      "%s", tvb_ip6_to_str(PNODE_POOL(tree), tvb, offset + 4));
            }
            proto_item_append_text(ti, "/%u", mask_len);
            break;
        }
