...
static int proto_bhttp = -1;
static int hf_bhttp_frame = -1;
static int hf_bhttp_framing_indicator = -1;
static int hf_bhttp_request_control_data = -1;
static int hf_bhttp_request_method_len = -1;
static int hf_bhttp_request_method = -1;
static int hf_bhttp_request_scheme_len = -1;
static int hf_bhttp_request_scheme = -1;
static int hf_bhttp_request_authority_len = -1;
static int hf_bhttp_request_authority = -1;
static int hf_bhttp_request_path_len = -1;
static int hf_bhttp_request_path = -1;
static int hf_bhttp_info_response_control_data = -1;
static int hf_bhttp_final_response_control_data = -1;
static int hf_bhttp_final_response_status = -1;
static int hf_bhttp_info_response_status = -1;
static int hf_bhttp_known_length_field_section = -1;
static int hf_bhttp_known_length_field_section_length = -1;
static int hf_bhttp_indeterminate_length_field_section = -1;
static int hf_bhttp_indeterminate_content_terminator = -1;
static int hf_bhttp_name_len = -1;
static int hf_bhttp_name = -1;
static int hf_bhttp_value_len = -1;
static int hf_bhttp_value = -1;
static int hf_bhttp_known_length_content = -1;
static int hf_bhttp_known_length_content_length = -1;
static int hf_bhttp_known_length_content_content = -1;
static int hf_bhttp_indeterminate_length_content = -1;
static int hf_bhttp_indeterminate_length_content_chunk_length = -1;
static int hf_bhttp_padding_length = -1;

/* Initialize the subtree pointers */
static gint ett_bhttp = -1;
static gint ett_bhttp_ft = -1;
static gint ett_bhttp_request_control_data = -1;
static gint ett_bhttp_info_response_control_data = -1;
static gint ett_bhttp_final_response_control_data = -1;
static gint ett_bhttp_known_length_field_section = -1;
static gint ett_bhttp_indeterminate_length_field_section = -1;
static gint ett_bhttp_response_control_data = -1;
static gint ett_bhttp_known_length_informational_response = -1;
static gint ett_bhttp_known_length_content = -1;
static gint ett_bhttp_indeterminate_length_content = -1;
...
{
    guint64 method_len, scheme_len, authority_len, path_len;
    gint32 lenvar;
    proto_tree *cd_tree;
    const guint8 *method, *path;
    ...
    proto_tree_add_item_ret_string(cd_tree, hf_bhttp_request_method, tvb, offset, (guint32)method_len, ENC_ASCII|ENC_NA, pinfo->pool, &method);
    offset += (guint32)method_len;

    proto_tree_add_item_ret_varint(cd_tree, hf_bhttp_request_scheme_len, tvb, offset, -1, ENC_VARINT_QUIC, &scheme_len, &lenvar);
    offset += lenvar;

    proto_tree_add_item(cd_tree, hf_bhttp_request_scheme, tvb, offset, (guint32)scheme_len, ENC_ASCII|ENC_NA);
    offset += (guint32)scheme_len;

    proto_tree_add_item_ret_varint(cd_tree, hf_bhttp_request_authority_len, tvb, offset, -1, ENC_VARINT_QUIC, &authority_len, &lenvar);
    offset += lenvar;

    proto_tree_add_item(cd_tree, hf_bhttp_request_authority, tvb, offset, (guint32)authority_len, ENC_ASCII|ENC_NA);
    offset += (guint32)authority_len;

    proto_tree_add_item_ret_varint(cd_tree, hf_bhttp_request_path_len, tvb, offset, -1, ENC_VARINT_QUIC, &path_len, &lenvar);
    offset += lenvar;

    proto_tree_add_item_ret_string(cd_tree, hf_bhttp_request_path, tvb, offset, (guint32)path_len, ENC_ASCII|ENC_NA, pinfo->pool, &path);
    offset += (guint32)path_len;
...
dissect_bhttp_known_field_section(tvbuff_t *tvb, proto_tree *tree, int offset)
{
    ...
    offset += lenvar;
    target_offset = offset + (guint32)fs_length;
    ...
    while(offset < target_offset){
        ...
        proto_tree_add_item(fs_tree, hf_bhttp_name, tvb, offset, (guint32)length, ENC_ASCII|ENC_NA);
        ...
        proto_tree_add_item(fs_tree, hf_bhttp_value, tvb, offset, (guint32)length, ENC_ASCII|ENC_NA);
        offset += (guint32)length;

        total_fields +=1;
    }

    proto_item_append_text(fs_tree, ": Length: %lu, Total Field lines: %u", fs_length, total_fields);
...
{
    guint64 length, content_terminator;
    gint32 lenvar;
    proto_tree *fs_tree;
    proto_item *pi;
...
{
    guint64 chunk_length, content_terminator;
    gint32 lenvar;
    proto_tree *c_tree;
    proto_item *pi;
    ...
    while (tvb_reported_length_remaining(tvb, offset) > 0) {
        ...
        if (chunk_length > 0) {
            proto_tree_add_item(c_tree, hf_bhttp_known_length_content_content, tvb, offset, (guint32)chunk_length, ENC_NA);
            offset += (guint32)chunk_length;
        }
...
dissect_bhttp_known_content(tvbuff_t *tvb, proto_tree *tree, int offset)
{
    ...
    if (length > 0) {
        proto_tree_add_item(c_tree, hf_bhttp_known_length_content_content, tvb, offset, (guint32)length, ENC_NA);
        offset += (guint32)length;
    }

    proto_item_append_text(c_tree, ": Length: %lu", length);
...
dissect_bhttp_informational_response_control_data(tvbuff_t *tvb, proto_tree *tree, int offset)
{
    ...
    proto_item_append_text(cd_tree, ": Status: %lu", status);
    proto_item_append_text(tree, ": Status: %lu", status);
    offset += lenvar;
