...
static int hf_msrcp_ext_src_addr = -1;
static int hf_msrcp_ext_dst_addr = -1;
...
dissect_msrcp(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    ...
    */
    ...
    col_add_fstr(pinfo->cinfo, COL_INFO, "%s ID %d (0x%X)",
        val_to_str(type, packettypenames, "TCP over MSRCP"), seq, seq);


    ti = proto_tree_add_item(tree, proto_msrcp, tvb, 0, -1, ENC_BIG_ENDIAN);
    proto_item_append_text(ti, "Type %s",
        val_to_str(type, packettypenames, "TCP over MSRCP"));
...
proto_register_msrcp(void)
{
    ...
    { &hf_msrcp_ext_src_addr,
        {"Source Address", "msrcp.nxt_srcaddr",
            FT_IPv6, BASE_NONE,
            NULL, 0x0,
            NULL, HFILL}
    },
    { &hf_msrcp_ext_dst_addr,
        {"Destination Address", "msrcp.nxt_dstaddr",
            FT_IPv6, BASE_NONE,
            NULL, 0x0,
            NULL, HFILL}
    }
