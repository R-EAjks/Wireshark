...
dissect_ipaddressport(int anchor, tvbuff_t *tvb, proto_tree *tree, guint16 offset)
{
  ...
  if (ipaddressport_type == IPADDRESSPORTTYPE_IPV4) {
    ...
    proto_item_append_text(ti_ipaddressport, " (%s:%d)", tvb_ip6_to_str(PNODE_POOL(tree), tvb, offset+2),tvb_get_ntohs(tvb,offset+2+16));
  }
  ...
  case IPADDRESSPORTTYPE_IPV6:
  {
    ...
    proto_item_append_text(ti_ipv6, ": %s:%d", tvb_ip6_to_str(PNODE_POOL(tree), tvb, offset),tvb_get_ntohs(tvb,offset+16));
