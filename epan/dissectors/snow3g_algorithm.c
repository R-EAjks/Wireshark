...
{
  // A faster iterative, rather than recursive, version from Pascal.
  u8 result, z;

  result = V;
  for(z = 1; z <= i; z++)
  {
    result = MULx(result,c);
  }
  return result;
}
...
static void Initialize(u32 k[4], u32 IV[4])
{
    u8 i=0;
    u32 F = 0x0;
    LFSR_S15 = k[3] ^ IV[0];
...
u8* snow3g_f9( u8* key, u32 count, u32 fresh, u32 dir, u8 *data, u64 length)
{
...
#if 0
    /* mrm added: Show inputs */
    printf("\n\nsnow3g_f9 (count=%u, fresh=%u, dir=%u, length=%llu): ", count, fresh, dir, length);
	...
	temp_MAC_I = ((u32)(EVAL >> 32) ^ z[4]);
	MAC_I[3] = temp_MAC_I & 0xff;
	MAC_I[2] = (temp_MAC_I >> 8) & 0xff;
	MAC_I[1] = (temp_MAC_I >> 16) & 0xff;
	MAC_I[0] = (temp_MAC_I >> 24) & 0xff;
