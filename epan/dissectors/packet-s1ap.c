...
#line 120 "./asn1/s1ap/packet-s1ap-template.c"
...
#line 165 "./asn1/s1ap/packet-s1ap-template.c"
...
dissect_s1ap_warningMessageContents(tvbuff_t *warning_msg_tvb, proto_tree *tree, packet_info *pinfo, guint8 dcs, int hf_nb_pages, int hf_decoded_page)
{
  ...
  for (i = 0, offset = 1; i < nb_of_pages; i++) {
    ...
    if (cb_data_tvb) {
      str = tvb_get_string_enc(pinfo->pool, cb_data_tvb, 0, tvb_reported_length(cb_data_tvb), ENC_UTF_8);
...
#line 1479 "./asn1/s1ap/s1ap.cnf"
  ...
  actx->created_item = proto_tree_add_item(tree, hf_index, parameter_tvb, 0, -1, ENC_UTF_8);
