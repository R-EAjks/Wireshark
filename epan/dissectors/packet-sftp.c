...
//                int pflags = tvb_get_ntohl(tvb, offset) ;
                ...
                proto_tree_add_item(sftp_tree, hf_ssh_sftp_handle, tvb, offset, slen, ENC_BIG_ENDIAN);
                offset += slen;
                ...
                offset += 4;
                proto_tree_add_item(sftp_tree, hf_ssh_sftp_data, tvb, offset, dlen, ENC_BIG_ENDIAN);
                offset += dlen;
...
//        }else if(typ==SSH_FXP_SYMLINK){
                ...
                offset += 4;
                proto_tree_add_item(sftp_tree, hf_ssh_lang_tag, tvb, offset, slen, ENC_BIG_ENDIAN);
                offset += slen;
                ...
                proto_tree_add_item(sftp_tree, hf_ssh_sftp_fxp_name_count, tvb, offset, 4, ENC_BIG_ENDIAN);
                offset += 4;
                guint cnt;
...
//    module_t *sftp_module;
    expert_module_t *expert_sftp;
    ...
    expert_sftp = expert_register_protocol(proto_sftp);
    expert_register_field_array(expert_sftp, ei, array_length(ei));

//    sftp_module = prefs_register_protocol(proto_sftp, NULL);
