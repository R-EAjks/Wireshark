...
#include "packet-extrememesh.h"

const char *gMeshNextProtocolStrings[MESH_NEXT_PROTOCOL_EOL] =
                                                  MESH_NEXT_PROTOCOL_STRINGS;
...
static gint dissect_extreme_eth(tvbuff_t *tvb, packet_info *pinfo,
                                 proto_tree *tree);

static void dissect_extreme_ps_areq(tvbuff_t *tvb, packet_info *pinfo,
                                     proto_tree *tree);

static gint dissect_extreme_eth_noaddr(tvbuff_t *tvb, packet_info *pinfo,
                                        proto_tree *tree);

static gint dissect_extreme_l2upd(tvbuff_t *tvb, packet_info *pinfo,
                                   proto_tree *tree);

static gint dissect_extreme_probe(tvbuff_t *tvb, packet_info *pinfo,
                                   proto_tree *tree);

static gint dissect_extreme_ps(tvbuff_t *tvb, packet_info *pinfo,
                                proto_tree *tree);
static void dissect_extreme_ps_arep(struct tvbuff *tvb, packet_info *pinfo,
                                     proto_tree *tree);
static void dissect_extreme_ps_breq(tvbuff_t *tvb, packet_info *pinfo,
                                     proto_tree *tree);
static void dissect_extreme_ps_brep(tvbuff_t *tvb, packet_info *pinfo,
                                     proto_tree *tree);
static void dissect_extreme_ps_bann(tvbuff_t *tvb, packet_info *pinfo,
                                     proto_tree *tree);
static void dissect_extreme_ps_bred(tvbuff_t *tvb, packet_info *pinfo,
                                     proto_tree *tree);
static void dissect_extreme_ps_sreq(tvbuff_t *tvb, packet_info *pinfo,
                                     proto_tree *tree);
static void dissect_extreme_ps_srep(tvbuff_t *tvb, packet_info *pinfo,
                                     proto_tree *tree);
static void dissect_extreme_ps_preq(tvbuff_t *tvb, packet_info *pinfo,
                                     proto_tree *tree);
static void dissect_extreme_ps_prep(tvbuff_t *tvb, packet_info *pinfo,
                                     proto_tree *tree);
static void dissect_extreme_ps_perr(tvbuff_t *tvb, packet_info *pinfo,
                                     proto_tree *tree);
static void dissect_extreme_ps_prst(tvbuff_t *tvb, packet_info *pinfo,
                                     proto_tree *tree);
static void dissect_extreme_ps_prem(tvbuff_t *tvb, packet_info *pinfo,
                                     proto_tree *tree);
static void dissect_extreme_ps_trace(tvbuff_t *tvb, packet_info *pinfo,
                                      proto_tree *tree);
static void dissect_extreme_ps_prer(tvbuff_t *tvb, packet_info *pinfo,
                                     proto_tree *tree);
                                     ...
                                     proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_arep, tvb, offset, tvb_captured_length(tvb), FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_arep_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_arep_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_arep_mpr_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_arep_orig_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_arep_opt_tot_len, tvb, offset, 2,
                        FALSE);
    offset+=2;
    while(tvb_captured_length(tvb) > offset)
    {
        option = tvb_get_guint8(tvb, offset);
        proto_tree_add_item(tree, hf_extreme_ps_arep_option, tvb, offset,
                                1, FALSE);
        offset++;
        if(option != 0) // Option 0 is a single padding byte, no length byte
        {
            proto_tree_add_item(tree, hf_extreme_ps_arep_option_len, tvb,
                                offset, 1, FALSE);
            offset++;
            switch(option)
            {
            case 4:
                proto_tree_add_item(tree, hf_extreme_ps_arep_result, tvb,
                                    offset, 1, FALSE);
                offset++;
                break;
            case 6:
                proto_tree_add_item(tree, hf_extreme_ps_arep_timeout, tvb,
                                    offset, 1, FALSE);
                offset++;
                break;
            default:
                                     ...
                                     proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_breq, tvb, offset,
                        tvb_captured_length(tvb), FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_breq_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_breq_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_breq_mpr_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_breq_orig_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_breq_opt_tot_len, tvb, offset, 2,
                        FALSE);
    offset+=2;
    while(tvb_captured_length(tvb) > offset)
    {
        option = tvb_get_guint8(tvb, offset);
        proto_tree_add_item(tree, hf_extreme_ps_breq_option, tvb, offset,
                                1, FALSE);
        offset++;
        if(option != 0) // Option 0 is a single padding byte, no length byte
        {
            proto_tree_add_item(tree, hf_extreme_ps_breq_option_len, tvb,
                                    offset, 1, FALSE);
            ...
            case 1:
                while(option_len > 0)
                {
                    proto_tree_add_item(tree, hf_extreme_ps_breq_proxy_addr,
                                        tvb, offset, 6, FALSE);
                    option_len-=6;
                    offset+=6;
                }
                break;
            case 2:
                proto_tree_add_item(tree, hf_extreme_ps_breq_old_mpr, tvb,
                                    offset, 6, FALSE);
                offset+=6;
                break;
            case 5:
                break;
            case 7:
                proto_tree_add_item(tree, hf_extreme_ps_breq_orig_pri, tvb,
                                    offset, 1, FALSE);
                offset++;
                break;
            case 8:
                while(option_len > 0)
                {
                    proto_tree_add_item(tree, hf_extreme_ps_breq_proxy_pri,
                                        tvb, offset, 1, FALSE);
                    option_len--;
                    offset++;
                }
                break;
            case 10:
                proto_tree_add_item(tree, hf_extreme_ps_breq_vlan_id, tvb,
                                    offset, 2, FALSE);
                offset+=2;
                break;
            case 11:
                while(option_len > 0)
                {
                    proto_tree_add_item(tree, hf_extreme_ps_breq_proxy_vlan_id,
                                        tvb, offset, 2, FALSE);
                    option_len-=2;
                    offset+=2;
                }
                break;
            case 12:
                proto_tree_add_item(tree, hf_extreme_ps_breq_seq, tvb,
                                    offset, 4, FALSE);
                offset+=4;
                break;
            default:
                                     ...
                                     proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_brep, tvb, offset,
                        tvb_captured_length(tvb), FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_brep_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_brep_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_brep_mpr_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_brep_orig_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_brep_opt_tot_len, tvb, offset, 2,
                        FALSE);
    offset+=2;
    while(tvb_captured_length(tvb) > offset)
    {
        option = tvb_get_guint8(tvb, offset);
        proto_tree_add_item(tree, hf_extreme_ps_brep_option, tvb, offset,
                                1, FALSE);
        offset++;
        if(option != 0) // Option 0 is a single padding byte, no length byte
        {
            ...
            {
            case 12:
                proto_tree_add_item(tree, hf_extreme_ps_brep_option_len, tvb,
                                    offset, 1, FALSE);
                offset++;
                proto_tree_add_item(tree, hf_extreme_ps_brep_seq, tvb,
                                    offset, 4, FALSE);
                offset+=4;
                break;
            default:
                                     ...
                                     proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_bann, tvb, offset,
                        tvb_captured_length(tvb), FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_bann_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_bann_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_bann_mpr_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_bann_orig_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_bann_opt_tot_len, tvb, offset, 2,
                        FALSE);
    offset+=2;
    while(tvb_captured_length(tvb) > offset)
    {
        option = tvb_get_guint8(tvb, offset);
        proto_tree_add_item(tree, hf_extreme_ps_bann_option, tvb, offset,
                                1, FALSE);
        offset++;
        if(option != 0) // Option 0 is a single padding byte, no length byte
        {
            option_len = tvb_get_guint8(tvb, offset);
            proto_tree_add_item(tree, hf_extreme_ps_bann_option_len, tvb,
                                offset, 1, FALSE);
            offset++;
            switch(option)
            {
            case 1:
                while(option_len > 0)
                {
                    proto_tree_add_item(tree, hf_extreme_ps_bann_proxy_addr,
                                        tvb, offset, 6, FALSE);
                    option_len-=6;
                    offset+=6;
                }
                break;
            case 2:
                proto_tree_add_item(tree, hf_extreme_ps_bann_old_root, tvb,
                                    offset, 6, FALSE);
                offset+=6;
                break;

            case 10:
                proto_tree_add_item(tree, hf_extreme_ps_bann_vlan_id, tvb,
                                    offset, 2, FALSE);
                offset+=2;
                break;
            case 12:
                proto_tree_add_item(tree, hf_extreme_ps_bann_seq, tvb,
                                    offset, 4, FALSE);
                offset+=4;
                break;
            default:
                                     ...
                                     proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_bred, tvb, offset,
                        tvb_captured_length(tvb), FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_bred_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_bred_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_bred_mpr_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_bred_orig_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_bred_opt_tot_len, tvb, offset, 2,
                        FALSE);
    offset+=2;
    while(tvb_captured_length(tvb) > offset)
    {
        option = tvb_get_guint8(tvb, offset);
        proto_tree_add_item(tree, hf_extreme_ps_bred_option, tvb, offset,
                                1, FALSE);
        offset++;
        if(option != 0) // Option 0 is a single padding byte, no length byte
        {
            proto_tree_add_item(tree, hf_extreme_ps_bred_option_len, tvb,
                                offset, 1, FALSE);
            offset++;
            switch(option)
            {
            case 12:
                proto_tree_add_item(tree, hf_extreme_ps_bred_seq, tvb,
                                    offset, 4, FALSE);
                offset+=4;
                break;
            default:
                                     ...
                                     proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_sreq, tvb, offset,
                        tvb_captured_length(tvb), FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_sreq_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_sreq_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_sreq_reserved, tvb, offset, 2,
                        FALSE);
    offset+=2;
    proto_tree_add_item(tree, hf_extreme_ps_sreq_orig_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_sreq_term_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_sreq_opt_tot_len, tvb, offset, 2,
                        FALSE);
    offset+=2;
    while(tvb_captured_length(tvb) > offset)
    {
        option = tvb_get_ntohs(tvb, offset);
        proto_tree_add_item(tree, hf_extreme_ps_sreq_option, tvb, offset,
                                2, FALSE);
        offset+=2;
        if(option != 0) // Option 0 is a single padding byte, no length byte
        {
            proto_tree_add_item(tree, hf_extreme_ps_sreq_option_len, tvb,
                                offset, 2, FALSE);
            offset+=2;
            switch(option)
            {
            case 10:
                proto_tree_add_item(tree, hf_extreme_ps_sreq_vlan_id, tvb,
                                    offset, 2, FALSE);
                offset+=2;
                break;
            default:
                                     ...
                                     proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_srep, tvb, offset,
                        tvb_captured_length(tvb), FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_srep_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_srep_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_srep_flags, tvb, offset, 1,
                        FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_srep_flags_reserved, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_srep_flags_status, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_srep_hop_count, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_srep_orig_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_srep_dest_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_srep_term_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_srep_opt_tot_len, tvb, offset, 2,
                        FALSE);
    offset+=2;
    while(tvb_captured_length(tvb) > offset)
    {
        option = tvb_get_ntohs(tvb, offset);
        proto_tree_add_item(tree, hf_extreme_ps_srep_option, tvb, offset,
                                2, FALSE);
        offset+=2;
        if(option != 0) // Option 0 is a single padding byte, no length byte
        {
            proto_tree_add_item(tree, hf_extreme_ps_srep_option_len, tvb,
                                offset, 2, FALSE);
            offset+=2;
            switch(option)
            {
            case 10:
                proto_tree_add_item(tree, hf_extreme_ps_srep_vlan_id, tvb,
                                    offset, 2, FALSE);
                offset+=2;
                break;
            default:
                                     ...
                                     proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_preq, tvb, offset,
                        tvb_captured_length(tvb), FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_preq_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_preq_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_preq_flags, tvb, offset, 1,
                        FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_preq_flags_broadcast, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_preq_flags_periodic, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_preq_flags_state, tvb, offset, 1,
                        FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_preq_flags_reserved, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_preq_flags_gratuitous, tvb,
                        offset, 1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_preq_flags_destination, tvb,
                         offset, 1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_preq_flags_unknown, tvb, offset,
                        1, FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_preq_hop_count, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_preq_ttl, tvb, offset, 4, FALSE);
    offset+=4;
    proto_tree_add_item(tree, hf_extreme_ps_preq_path_metrics, tvb, offset, 2,
                        FALSE);
    offset+=2;
    proto_tree_add_item(tree, hf_extreme_ps_preq_services, tvb, offset, 1,
                        FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_preq_services_reserved, tvb,
                        offset, 1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_preq_services_mobile, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_preq_services_path_pref, tvb,
                        offset, 1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_preq_services_geo, tvb, offset, 1,
                        FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_preq_services_proxy, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_preq_services_root, tvb, offset,
                        1, FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_preq_reserved, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_preq_id, tvb, offset, 4, FALSE);
    offset+=4;
    proto_tree_add_item(tree, hf_extreme_ps_preq_term_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_preq_dest_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_preq_dest_seq, tvb, offset, 4,
                        FALSE);
    offset+=4;
    proto_tree_add_item(tree, hf_extreme_ps_preq_orig_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_preq_orig_seq, tvb, offset, 4,
                        FALSE);
    offset+=4;
    proto_tree_add_item(tree, hf_extreme_ps_preq_opt_tot_len, tvb, offset, 2,
                        FALSE);
    offset+=2;
    while(tvb_captured_length(tvb) > offset)
    {
        option = tvb_get_ntohs(tvb, offset);
        proto_tree_add_item(tree, hf_extreme_ps_preq_option, tvb, offset,
                                2, FALSE);
        offset+=2;
        if(option != 0) // Option 0 is a single padding byte, no length byte
        {
            option_len = tvb_get_ntohs(tvb, offset);
            proto_tree_add_item(tree, hf_extreme_ps_preq_option_len, tvb,
                                offset, 2, FALSE);
            offset+=2;
            switch(option)
            {
            case 1:
                while(option_len > 0)
                {
                    proto_tree_add_item(tree, hf_extreme_ps_preq_mcast_sub,
                                        tvb, offset, 6, FALSE);
                    option_len-=6;
                    offset+=6;
                }
                break;
            case 10:
                proto_tree_add_item(tree, hf_extreme_ps_preq_vlan_id, tvb,
                                    offset, 2, FALSE);
                offset+=2;
                break;
            case 14:
                proto_tree_add_item(tree, hf_extreme_ps_preq_mint_id, tvb,
                                    offset, 4, FALSE);
                offset+=4;
                break;
            default:
                                     ...
                                     proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_prep, tvb, offset,
                        tvb_captured_length(tvb), FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_prep_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prep_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prep_flags, tvb, offset, 1,
                        FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_prep_flags_reserved, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_prep_flags_new_route, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_prep_flags_repair, tvb, offset, 1,
                        FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_prep_flags_ack, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prep_hop_count, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prep_path_metrics, tvb, offset, 2,
                        FALSE);
    offset+=2;
    proto_tree_add_item(tree, hf_extreme_ps_prep_services, tvb, offset, 1,
                        FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_prep_services_reserved, tvb,
                        offset, 1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_prep_services_mobile, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_prep_services_path_pref, tvb,
                        offset, 1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_prep_services_geo, tvb, offset, 1,
                        FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_prep_services_proxy, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_prep_services_root, tvb, offset,
                        1, FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prep_reserved, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prep_term_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_prep_dest_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_prep_dest_seq, tvb, offset, 4,
                        FALSE);
    offset+=4;
    proto_tree_add_item(tree, hf_extreme_ps_prep_orig_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_prep_orig_seq, tvb, offset, 4,
                        FALSE);
    offset+=4;
    proto_tree_add_item(tree, hf_extreme_ps_prep_lifetime, tvb, offset, 4,
                        FALSE);
    offset+=4;
    proto_tree_add_item(tree, hf_extreme_ps_prep_opt_tot_len, tvb, offset, 2,
                        FALSE);
    offset+=2;
    while(tvb_captured_length(tvb) > offset)
    {
        option = tvb_get_ntohs(tvb, offset);
        proto_tree_add_item(tree, hf_extreme_ps_prep_option, tvb, offset,
                                2, FALSE);
        offset+=2;
        if(option != 0) // Option 0 is a single padding byte, no length byte
        {
            option_len = tvb_get_ntohs(tvb, offset);
            proto_tree_add_item(tree, hf_extreme_ps_prep_option_len, tvb,
                                offset, 2, FALSE);
            offset+=2;
            switch(option)
            {
            case 1:
                while(option_len > 0)
                {
                    proto_tree_add_item(tree, hf_extreme_ps_prep_mcast_sub,
                                        tvb, offset, 6, FALSE);
                    option_len-=6;
                    offset+=6;
                }
                break;
            case 10:
                proto_tree_add_item(tree, hf_extreme_ps_prep_vlan_id, tvb,
                                    offset, 2, FALSE);
                offset+=2;
                break;
            case 14:
                proto_tree_add_item(tree, hf_extreme_ps_prep_mint_id, tvb,
                                    offset, 4, FALSE);
                offset+=4;
                break;
            default:
                                     ...
                                     proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_perr, tvb, offset, tvb_captured_length(tvb),
                        FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_perr_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_perr_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_perr_flags, tvb, offset, 1,
                        FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_perr_flags_reserved, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_perr_flags_warning, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_perr_flags_no_delete, tvb, offset,
                        1, FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_perr_dest_count, tvb, offset, 1,
                        FALSE);
    offset++;
    while (dst_cnt-- > 0)
    {
        proto_tree_add_item(tree, hf_extreme_ps_perr_unrch_dest, tvb, offset,
                            6, FALSE);
        offset+=6;
        proto_tree_add_item(tree, hf_extreme_ps_perr_unrch_dest_seq,
                            tvb, offset, 4, FALSE);
        offset+=4;
    }
}
                                     ...
                                     proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_prst, tvb, offset,
                        tvb_captured_length(tvb), FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_prst_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prst_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prst_hops_to_live, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prst_reserved, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prst_id, tvb, offset, 4,
                        FALSE);
    offset+=4;
    proto_tree_add_item(tree, hf_extreme_ps_prst_orig_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_prst_dest_addr, tvb, offset, 6,
                        FALSE);
}
                                     ...
                                     proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_prem, tvb, offset,
                        tvb_captured_length(tvb), FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_prem_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prem_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prem_mpr_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_prem_orig_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_prem_opt_tot_len, tvb, offset, 2,
                        FALSE);
    offset+=2;
    while(tvb_captured_length(tvb) > offset)
    {
        option = tvb_get_guint8(tvb, offset);
        proto_tree_add_item(tree, hf_extreme_ps_prem_option, tvb, offset,
                                1, FALSE);
        offset++;
        if(option != 0) // Option 0 is a single padding byte, no length byte
        {
            option_len = tvb_get_guint8(tvb, offset);
            proto_tree_add_item(tree, hf_extreme_ps_prem_option_len, tvb,
                                offset, 1, FALSE);
            offset++;
            switch(option)
            {
            case 1:
                while(option_len > 0)
                {
                    proto_tree_add_item(tree, hf_extreme_ps_prem_proxy_addr,
                                        tvb, offset, 6, FALSE);
                    option_len-=6;
                    offset+=6;
                }
            ...
            case 11:
                while(option_len > 0)
                {
                    proto_tree_add_item(tree, hf_extreme_ps_prem_proxy_vlan_id,
                                        tvb, offset, 2, FALSE);
                    option_len-=2;
                    offset+=2;
                }
                                      ...
                                      proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_trace, tvb, offset,
                        tvb_captured_length(tvb), FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_trace_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_trace_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_trace_flags, tvb, offset, 1,
                        FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_trace_flags_reserved, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_trace_flags_reply, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_trace_flags_no_path, tvb, offset,
                        1, FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_trace_dest_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_trace_orig_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_trace_hop_count, tvb, offset, 1,
                        FALSE);
    offset++;
    while(hop_cnt-- > 0)
    {
        proto_tree_add_item(tree, hf_extreme_ps_trace_addl_path, tvb, offset,
                            6, FALSE);
        offset+=6;
    }
}
                                     ...
                                     proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_prer, tvb, offset,
                        tvb_captured_length(tvb), FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_prer_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prer_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prer_dest_count, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prer_reserved, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_prer_orig_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_prer_dest_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_prer_unrch_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_prer_opt_tot_len, tvb, offset, 2,
                        FALSE);
    offset+=2;
    while(tvb_captured_length(tvb) > offset)
    {
        option = tvb_get_ntohs(tvb, offset);
        proto_tree_add_item(tree, hf_extreme_ps_prer_option, tvb, offset,
                                2, FALSE);
        offset+=2;
        if(option != 0) // Option 0 is a single padding byte, no length byte
        {
            proto_tree_add_item(tree, hf_extreme_ps_prer_option_len, tvb,
                                offset, 2, FALSE);
            offset+=2;
            switch(option)
            {
            case 11:
                proto_tree_add_item(tree, hf_extreme_ps_prer_vlan_id, tvb,
                                    offset, 2, FALSE);
                offset+=2;
                break;
            default:
...
static gint dissect_extreme_mch(tvbuff_t *tvb, packet_info *pinfo,
                                 proto_tree *tree)
{
    ...
    proto_tree_add_item(meshTree, proto_extreme_mch, tvb, offset, tvb_captured_length(tvb),
                        FALSE);
    proto_tree_add_item(meshTree, hf_extreme_mch_version, tvb, offset, 1,
                        FALSE);
    offset++;
    next_proto = tvb_get_guint8(tvb, offset);
    proto_tree_add_item(meshTree, hf_extreme_mch_next_proto, tvb, offset, 1,
                        FALSE);
    offset++;
    ...
    offset++;
    proto_tree_add_item(meshTree, hf_extreme_mch_priority, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(meshTree, hf_extreme_mch_usr_pri_flags, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(meshTree, hf_extreme_mch_usr_pri_flags_user_priority,
                        tvb, offset, 1, FALSE);
    proto_tree_add_item(meshTree, hf_extreme_mch_usr_pri_flags_reserved, tvb,
                        offset, 1, FALSE);
    proto_tree_add_item(meshTree, hf_extreme_mch_usr_pri_flags_from_wan, tvb,
                        offset, 1, FALSE);
    proto_tree_add_item(meshTree, hf_extreme_mch_usr_pri_flags_to_wan, tvb,
                        offset, 1, FALSE);
    proto_tree_add_item(meshTree, hf_extreme_mch_usr_pri_flags_forward, tvb,
                        offset, 1, FALSE);
    offset++;
    proto_tree_add_item(meshTree, hf_extreme_mch_sequence, tvb, offset, 2,
                        FALSE);
    offset+=2;
    ...
    while(next_proto != (gint)MESH_NEXT_PROTOCOL_INVALID)
    {
        ...
        case MESH_NEXT_PROTOCOL_ENCAPSULATED_ETH:
            next_proto = dissect_extreme_eth(nextTvb, pinfo, meshTree);
            break;
...
static int dissect_extrememesh(tvbuff_t *tvb, packet_info *pinfo,
                                 proto_tree *tree, void *data _U_)
{
    ...
    tvbuff_t *next_tvb = NULL;
        ...
        ti = proto_tree_add_item(tree, proto_extreme_mesh, tvb, offset, -1,
                                 FALSE);
        meshTree = proto_item_add_subtree(ti, ett_extreme_mesh);
        proto_tree_add_item(meshTree, hf_extreme_mesh_version, tvb, offset,
                            1, FALSE);
        offset++;
        next_proto = tvb_get_guint8(tvb, offset);
        proto_tree_add_item(meshTree, hf_extreme_mesh_nextproto, tvb, offset,
                            1, FALSE);
        offset++;
        ...
        while(next_proto != (gint)MESH_NEXT_PROTOCOL_INVALID)
        {
            ...
            case MESH_NEXT_PROTOCOL_MCH:
                ...
                break;
            case MESH_NEXT_PROTOCOL_ENCAPSULATED_ETH:
                next_proto = dissect_extreme_eth(next_tvb, pinfo, meshTree);
                break;
            case MESH_NEXT_PROTOCOL_PS:
            ...
            case MESH_NEXT_PROTOCOL_EAPOM:
            case MESH_NEXT_PROTOCOL_ENCAPSULATED_ETH_NO_ADDR:
                next_proto = dissect_extreme_eth_noaddr(next_tvb, pinfo,
                                                         meshTree);
                break;
            case MESH_NEXT_PROTOCOL_L2_UPDATE:
...
static gint dissect_extreme_eth(tvbuff_t *tvb, packet_info *pinfo,
                                 proto_tree *tree)
{
    //These are encapsulated ethernet frames....
    dissector_handle_t ethDis = find_dissector("eth_withoutfcs");

    if(ethDis)
    {
        call_dissector(ethDis, tvb, pinfo, tree);
    }
                                     ...
                                     proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_ps_areq, tvb, offset,
                        tvb_captured_length(tvb), FALSE);
    proto_tree_add_item(tree, hf_extreme_ps_areq_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_areq_frame_type, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_ps_areq_mpr_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_areq_orig_addr, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_ps_areq_opt_tot_len, tvb, offset, 2,
                        FALSE);
    offset+=2;
    while(tvb_captured_length(tvb) > offset)
    {
        option = tvb_get_guint8(tvb, offset);
        proto_tree_add_item(tree, hf_extreme_ps_areq_option, tvb, offset,
                                1, FALSE);
        offset++;
        if(option != 0) // Option 0 is a single padding byte, no length byte
        {
            proto_tree_add_item(tree, hf_extreme_ps_areq_option_len, tvb,
                                offset, 1, FALSE);
            offset++;
            switch(option)
            {
            case 2:
                proto_tree_add_item(tree, hf_extreme_ps_areq_old_mpr, tvb,
                                    offset, 6, FALSE);
                offset+=6;
                break;
            case 3:
                proto_tree_add_item(tree, hf_extreme_ps_areq_proxies, tvb,
                                    offset, 1, FALSE);
                offset++;
                break;
            default:
                                        ...
                                        proto_tree *tree)
{
    ...
    dissect_extreme_eth(nextTvb, pinfo, tree);
                                   ...
                                   proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_l2upd, tvb, offset, tvb_captured_length(tvb),
                        FALSE);
    proto_tree_add_item(tree, hf_extreme_l2upd_proxy_owner, tvb, offset, 6,
                        FALSE);
    offset+=6;
    proto_tree_add_item(tree, hf_extreme_l2upd_ballast, tvb, offset,
                        tvb_captured_length(tvb)-6, FALSE);
                                   ...
                                   proto_tree *tree)
{
    ...
    proto_tree_add_item(tree, proto_extreme_probe, tvb, offset,
                        12+ballast_len, FALSE);
    proto_tree_add_item(tree, hf_extreme_probe_version, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_probe_op_code, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_probe_flags, tvb, offset, 1, FALSE);
    proto_tree_add_item(tree, hf_extreme_probe_flags_reserved, tvb, offset,
                        1, FALSE);
    proto_tree_add_item(tree, hf_extreme_probe_flags_reply, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_probe_priority, tvb, offset, 1,
                        FALSE);
    offset++;
    proto_tree_add_item(tree, hf_extreme_probe_job_id, tvb, offset, 2, FALSE);
    offset+=2;
    proto_tree_add_item(tree, hf_extreme_probe_sequence, tvb, offset, 4,
                        FALSE);
    offset+=4;
    proto_tree_add_item(tree, hf_extreme_probe_ballast_len, tvb, offset, 2,
                        FALSE);
    offset+=2;
    proto_tree_add_item(tree, hf_extreme_probe_ballast, tvb, offset,
                        ballast_len, FALSE);
...
void proto_register_extrememesh(void)
{
   ...
   { &hf_extreme_probe_flags_reply, {
    ...
    module_t *extrememesh_module;

    /* registration */
    /* extreme mesh */
    proto_extreme_mesh = proto_register_protocol("Extreme Mesh",
                                 "EXTREME MESH", "extreme_mesh");
    proto_register_field_array(proto_extreme_mesh, hf_extreme_mesh,
                               array_length(hf_extreme_mesh));
    proto_register_subtree_array(ett, array_length(ett));

    /* extreme mesh control header */
    proto_extreme_mch = proto_register_protocol(
             "Extreme Mesh Control Header", "EXTREME MCH", "extreme_mch");
    proto_register_field_array(proto_extreme_mch, hf_extreme_mch,
                               array_length(hf_extreme_mch));



    /* extreme hello */
    proto_extreme_hello = proto_register_protocol("Extreme Hello",
                                 "EXTREME HELLO", "extreme_hello");
    proto_register_field_array(proto_extreme_hello, hf_extreme_hello,
                               array_length(hf_extreme_hello));

    /* extreme security */
    proto_extreme_security = proto_register_protocol("Extreme Security",
                                 "EXTREME SECURITY", "extreme_security");
    proto_register_field_array(proto_extreme_security, hf_extreme_security,
                               array_length(hf_extreme_security));

    /* extreme contention free period (CFP) update */
    proto_extreme_cfpu = proto_register_protocol("Extreme Cfpu",
                                 "EXTREME CFPU", "extreme_cfpu");
    proto_register_field_array(proto_extreme_cfpu, hf_extreme_cfpu,
                               array_length(hf_extreme_cfpu));

    /* extreme EAP over mesh */
    proto_extreme_eapom = proto_register_protocol("Extreme EAPOM",
                                 "EXTREME EAPOM", "extreme_eapom");
    proto_register_field_array(proto_extreme_eapom, hf_extreme_eapom,
                               array_length(hf_extreme_eapom));

    /* extreme mesh L2 update */
    proto_extreme_l2upd = proto_register_protocol(
            "Extreme Mesh L2 Update", "EXTREME L2UPD", "extreme_l2upd");
    proto_register_field_array(proto_extreme_l2upd, hf_extreme_l2upd,
                               array_length(hf_extreme_l2upd));
    /* extreme mesh probe message */
    proto_extreme_probe = proto_register_protocol(
            "Extreme Mesh Probe Message", "EXTREME PROBE", "extreme_probe");
    proto_register_field_array(proto_extreme_probe, hf_extreme_probe,
                               array_length(hf_extreme_probe));

    /* extreme mesh path selection authorization request */
    proto_extreme_ps_areq = proto_register_protocol(
        "Extreme Mesh Path Selection Authorization Request",
        "EXTREME PS AREQ", "extreme_ps_areq");
    proto_register_field_array(proto_extreme_ps_areq, hf_extreme_ps_areq,
                               array_length(hf_extreme_ps_areq));

    /* extreme mesh path selection authorization reply */
    proto_extreme_ps_arep = proto_register_protocol(
        "Extreme Mesh Path Selection Authorization Reply",
        "EXTREME PS AREP", "extreme_ps_arep");
    proto_register_field_array(proto_extreme_ps_arep, hf_extreme_ps_arep,
                               array_length(hf_extreme_ps_arep));

    /* extreme mesh path selection bind request */
    proto_extreme_ps_breq = proto_register_protocol(
        "Extreme Mesh Path Selection Bind Request",
        "EXTREME PS BREQ", "extreme_ps_breq");
    proto_register_field_array(proto_extreme_ps_breq, hf_extreme_ps_breq,
                               array_length(hf_extreme_ps_breq));

    /* extreme mesh path selection bind reply */
    proto_extreme_ps_brep = proto_register_protocol(
        "Extreme Mesh Path Selection Bind Reply",
        "EXTREME PS BREP", "extreme_ps_brep");
    proto_register_field_array(proto_extreme_ps_brep, hf_extreme_ps_brep,
                               array_length(hf_extreme_ps_brep));

    /* extreme mesh path selection bind announcement */
    proto_extreme_ps_bann = proto_register_protocol(
        "Extreme Mesh Path Selection Bind Announcement",
        "EXTREME PS BANN", "extreme_ps_bann");
    proto_register_field_array(proto_extreme_ps_bann, hf_extreme_ps_bann,
                               array_length(hf_extreme_ps_bann));

    /* extreme mesh path selection bind removed */
    proto_extreme_ps_bred = proto_register_protocol(
        "Extreme Mesh Path Selection Bind Removed",
        "EXTREME PS BRED", "extreme_ps_bred");
    proto_register_field_array(proto_extreme_ps_bred, hf_extreme_ps_bred,
                               array_length(hf_extreme_ps_bred));

    /* extreme mesh path selection status request */
    proto_extreme_ps_sreq = proto_register_protocol(
        "Extreme Mesh Path Selection Status Request", "EXTREME PS SREQ",
                           "extreme_ps_sreq");
    proto_register_field_array(proto_extreme_ps_sreq, hf_extreme_ps_sreq,
                               array_length(hf_extreme_ps_sreq));

    /* extreme mesh path selection status reply */
    proto_extreme_ps_srep = proto_register_protocol(
        "Extreme Mesh Path Selection Status Reply", "EXTREME PS SREP",
                           "extreme_ps_srep");
    proto_register_field_array(proto_extreme_ps_srep, hf_extreme_ps_srep,
                               array_length(hf_extreme_ps_srep));

    /* extreme mesh path selection path request */
    proto_extreme_ps_preq = proto_register_protocol(
        "Extreme Mesh Path Selection Path Request", "EXTREME PS PREQ",
                           "extreme_ps_preq");
    proto_register_field_array(proto_extreme_ps_preq, hf_extreme_ps_preq,
                               array_length(hf_extreme_ps_preq));

    /* extreme mesh path selection path reply */
    proto_extreme_ps_prep = proto_register_protocol(
        "Extreme Mesh Path Selection Path Reply", "EXTREME PS PREP",
                           "extreme_ps_prep");
    proto_register_field_array(proto_extreme_ps_prep, hf_extreme_ps_prep,
                               array_length(hf_extreme_ps_prep));

    /* extreme mesh path selection path error */
    proto_extreme_ps_perr = proto_register_protocol(
        "Extreme Mesh Path Selection Path Error", "EXTREME PS PERR",
                           "extreme_ps_perr");
    proto_register_field_array(proto_extreme_ps_perr, hf_extreme_ps_perr,
                               array_length(hf_extreme_ps_perr));

    /* extreme mesh path selection path reset */
    proto_extreme_ps_prst = proto_register_protocol(
        "Extreme Mesh Path Selection Path Reset", "EXTREME PS PRST",
                           "extreme_ps_prst");
    proto_register_field_array(proto_extreme_ps_prst, hf_extreme_ps_prst,
                               array_length(hf_extreme_ps_prst));

    /* extreme mesh path selection proxy remove */
    proto_extreme_ps_prem = proto_register_protocol(
        "Extreme Mesh Path Selection Proxy Remove", "EXTREME PS PREM",
                           "extreme_ps_prem");
    proto_register_field_array(proto_extreme_ps_prem, hf_extreme_ps_prem,
                               array_length(hf_extreme_ps_prem));

    /* extreme mesh path selection trace path */
    proto_extreme_ps_trace = proto_register_protocol(
        "Extreme Mesh Path Selection Trace Path", "EXTREME PS TRACE",
                           "extreme_ps_trace");
    proto_register_field_array(proto_extreme_ps_trace, hf_extreme_ps_trace,
                               array_length(hf_extreme_ps_trace));

    /* extreme mesh path selection proxy error */
    proto_extreme_ps_prer = proto_register_protocol(
        "Extreme Mesh Path Selection Proxy Error", "EXTREME PS PRER",
                           "extreme_ps_prer");
    proto_register_field_array(proto_extreme_ps_prer, hf_extreme_ps_prer,
                               array_length(hf_extreme_ps_prer));

    /* add a user preference to set the Extreme Mesh ethertype */
    extrememesh_module = prefs_register_protocol(proto_extreme_mesh,
                            proto_register_handoff_extrememesh);
	if (extrememesh_module) {
	}
}
...
void proto_register_handoff_extrememesh(void)
{
    ...
    {
        extrememesh_handle = create_dissector_handle(dissect_extrememesh,
                                                      proto_extreme_mesh);
        dissector_add_uint("ethertype", ETHERTYPE_IEEE_EXTREME_MESH,
                      extrememesh_handle);
        prefs_initialized = TRUE;
    }
    else
    {
        dissector_delete_uint("ethertype", ETHERTYPE_IEEE_EXTREME_MESH,
                         extrememesh_handle);
    }
}
