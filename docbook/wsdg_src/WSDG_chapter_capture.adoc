...
Extcap code can be run by Wireshark or directly by command line/console. In both cases
the code can require to react to user signals (e.g. Ctrl+C) to cleanup when stopped.

For extcaps in C there is `extcap/extcap-base.h` call `extcap_base_set_signal_handlers()`
which registers handlers for common signals on OS (Linux/MAC/Windows). When application
is stopped, global variable `extcap_end_application` is set to `TRUE`. Therefore
application can check it e.g. in main loop whether it should stop.
...
int main(int argc, char *argv[])
{
    ...
    /* Register handlers for console signals */
    if (!extcap_base_set_signal_handlers())
        goto end;

    /* Setup what should be cleaned up later, e.g. configure a device */
    setup();
    ...
    /* Main loop */
    {
        ...
        /* Write it to Wireshark */
        libpcap_write_packet(...);

        /* Loop till extcap_end_application is set by signal handler */
...
[NOTE]
Please note that signals works just on command line/console. To allow extcap cleanup
when run by Wireshark see <<ChCaptureExtcapShutdown>>.


[[ChCaptureExtcapShutdown]]

==== Extcap shutdown

An extcap utility is terminated by wireshark when capture is stopped. Termination of
the extcap depends on OS (kill(), TerminateProcess(), ...) and the extcap has no chance
to catch this.

Wireshark implements shutdown pipe for this case. When capture is stopped, Wireshark
writes word `QUIT` to the pipe. Extcap should read the pipe and when command is received
it should cleanup and quit. Wireshark waits 5000 ms till the extcap is terminated as usually.

To notify Wireshark that extcap is capable of shutdown pipe, it should exhibit `shutdown`
keyword in configuration query.

.Example of interface definition with shutdown pipeline
[source,bash]
----
...
When extcap is shutdown capable, it will receive `--shutdown <pipe>` argument during
start.

For extcaps in C `extcap/extcap-base.h` contains `extcap_base_register_shutdown_pipe()`
call to notify framework that it is shutdown pipe capable. When the call is used,
framework starts thread which monitors the pipe and sets global variable
`extcap_end_application` to `TRUE` when `QUIT` is sent from Wireshark.
...
int main(int argc, char *argv[])
{
    ...
    /* Init extcap_base configuration */
    help_url = data_file_url("example.html");
    ...
    /* extcap supports shutdown pipe */
    extcap_base_register_shutdown_pipe(extcap_conf);
    ...
    while ((result = ws_getopt_long(argc, argv, ":", longopts, &option_idx)) != -1) {
        ...
        case OPT_...:
            ...
        default:
            /* Process any extcap_base parameter including shutdown pipe */
    ...
    /* Register handlers for console signals.
     * It is not required for shutdown pipe case, but allows extcap to run from
